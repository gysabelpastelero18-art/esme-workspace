...
const columns = [
  'item', 'beg', 'production', 'cashier', 'bar', 'kitchen', 'oneBalete', 'sold', 'spoilage', 'return', 'short', 'over', 'ending'
];

const branchOptions = ['Mayon Branch', 'One Balete Branch'];

export default function BakeryInvPage() {
  // Print handler
  const handlePrint = () => {
    window.print();
  };
  ...
  const [bakeryData, setBakeryData] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    setLoading(true);
    loadProductionData(selectedDate, selectedBranch, 'Bakery').then(result => {
      if (result.success && result.data && Array.isArray(result.data.data)) {
        setBakeryData(result.data.data);
      } else {
        setBakeryData([]);
      }
      setLoading(false);
  ...
  <div style={{ padding: '16px 4px', fontFamily: 'Arial, sans-serif', background: 'rgba(255,255,255,0.95)', minHeight: '100vh' }}>
      ...
      <div className="bakery-print-title" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 4 }}>
        <img src="/images/logo.png" alt="Logo" style={{ height: 60, marginRight: 24 }} />
        <h1 style={{ color: '#4A6741', fontWeight: 800, fontSize: 32, margin: 0 }}>Bakery Daily Inventory</h1>
      </div>
      {/* Print-only branch, department, date under title */}
      <div className="bakery-print-info" style={{ display: 'none', textAlign: 'center', fontWeight: 600, color: '#4A6741', fontSize: 16, marginBottom: 16 }}>
        {selectedBranch} - Bakery&nbsp;
        {(() => {
      ...
      <div className="no-print" style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 32, justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg, rgba(156,175,136,0.15), rgba(74,103,65,0.1))', padding: '8px 16px', borderRadius: 12, border: '1px solid rgba(156,175,136,0.3)', backdropFilter: 'blur(10px)' }}>
            ...
            <span style={{ fontWeight: 700, color: '#2E7D32', fontSize: 18 }}>Bakery</span>
          </div>
        </div>
        <button
          onClick={handlePrint}
  ...
  {/* Print-only simple text for date, branch, department removed as requested */}
      {/* Table */}
  <div className="bakery-print-table" style={{ background: 'white', borderRadius: 20, border: '1px solid #e0e0e0', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', padding: '16px 0' }}>
  ...
  {columns.map(col => {
    const sumColumns = [
      'beg', 'production', 'cashier', 'bar', 'kitchen', 'oneBalete', 'sold', 'spoilage', 'return', 'short', 'over', 'ending'
    ];
    ...
    const sum = bakeryData.reduce((acc, row) => {
      const val = row[col];
    ...
    <style>{`
      @media print {
        ...
        .bakery-print-title, .bakery-print-title *,
        .bakery-print-table, .bakery-print-table *,
        .bakery-print-info, .bakery-print-info * {
