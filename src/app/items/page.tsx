...
const columns = [
  "MID", "MDate", "EKCode", "Barcode", "ItemCode", "FoodGroup", "FGCode", "Category", "FCCode", "Subcategory", "FSCCode", "Department", "Supplier", "ItemDesc", "Item", "Brand", "UOMQty", "Unit", "CUOMQty", "CUnit", "Location", "Storage", "Container", "ForBarcode", "EncodedBy", "Status", "ForInv", "EkNum", "BGroup", "PCSPerPack", "PricePc", "Price", "UInv"
];

export default function ItemsPage() {
  const handleReset = async () => {
    if (window.confirm('Are you sure you want to delete all items from the database?')) {
      setItems([]);
  ...
  // Fetch items from database by search
  useEffect(() => {
    if (search) {
      fetch(`/api/items?itemdesc=${encodeURIComponent(search)}`)
        .then(res => res.json())
        .then(data => {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const cleaned = imported.map((row: any) => {
        if (row.MDate) {
          const fixed = fixDate(row.MDate);
          // Convert mm/dd/yyyy to ISO string for Prisma
          if (typeof fixed === 'string' && fixed.match(/^\d{2}\/\d{2}\/\d{4}$/)) {
            const [mm, dd, yyyy] = fixed.split('/');
            row.MDate = new Date(`${yyyy}-${mm}-${dd}`).toISOString();
          } else if (typeof fixed === 'string' && !isNaN(Date.parse(fixed))) {
            row.MDate = new Date(fixed).toISOString();
          } else {
            row.MDate = fixed;
          }
        }
        // Convert ForBarcode to Boolean or Null
        if (typeof row.ForBarcode !== 'boolean') {
          row.ForBarcode = null;
        }
        // Convert ForInv to Boolean or Null
        if (typeof row.ForInv !== 'boolean') {
          row.ForInv = null;
        }
        // Convert EkNum to string if not already
        if (row.EkNum !== undefined && typeof row.EkNum !== 'string') {
          row.EkNum = String(row.EkNum);
        }
        return row;
      });
      ...
      if (result.success) {
        ...
        // Reload items from database to ensure dropdown uses Prisma data
        fetch('/api/items')
          .then(res => res.json())
            ...
            <table className="min-w-[1200px] w-full border border-gray-300 rounded-xl overflow-hidden shadow-sm text-xs sm:text-sm">
              ...
              <tbody>
                ...
                {items
                  .filter(row =>
                    !search ||
                    (row.ItemDesc && row.ItemDesc.toLowerCase().includes(search.toLowerCase()))
                  )
                  .map((row, idx) => (
                    <tr
                      key={idx}
