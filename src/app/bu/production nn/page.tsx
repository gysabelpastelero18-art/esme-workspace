...
import { saveSales } from '@/lib/sales-firebase';
...
export default function Production() {
  ...
  const getTodayDate = () => {
    ...
    // Helper: Calculate sold quantity for bakery items based on salesRows and selectedDate
    const getSoldQty = (itemName: string) => {
      return salesRows
        .filter(
          (row: any) =>
            row.category?.toLowerCase() === 'bakery' &&
            row.productname?.toLowerCase() === itemName.toLowerCase() &&
            row.edate === selectedDate
        )
  ...
  const fileInputSalesRef = useRef<HTMLInputElement>(null);
  ...
  parseFile(file, async (rows) => {
    try {
      if (!rows || rows.length === 0) {
        setErrorMsg('Error importing sales: File could not be parsed.');
        return;
      }
      ...
      const normalizedRows = rows.map((row: any) => ({
        ...
        ototal: Number(row.ototal) || 0,
      }));
      await saveSales(normalizedRows);
      setErrorMsg(`Successfully imported ${normalizedRows.length} sales.`);
      setSalesRows(normalizedRows);
    } catch (err) {
      setErrorMsg('Error importing sales: ' + (err?.toString() || 'Unknown error'));
    }
  });
  ...
  // Fields to block mouse wheel increment/decrement
  const wheelBlockFields = [
    'beg', 'production', 'del', 'used', 'cashier', 'bar', 'kitchen', 'oneBalete', 'foodtrays', 'event', 'spoilage', 'return', 'short', 'over'
  ];

  // Keyboard navigation for Excel-like behavior
  const handleCellKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, rowIdx: number, colIndex: number) => {
    ...
    if (e.key === 'ArrowRight') nextCol = Math.min(editableFields.length - 1, colIndex + 1);
                ...
                <tbody>
                  {productionData.map((row, rowIdx) => (
                    <tr key={row.id} style={{ borderBottom: '1px solid #e0e0e0' }}>
                      ...
                      {editableFields.map((field, colIndex) => {
                        ...
                        return (
                          <td key={field as string} style={{ textAlign: 'center', borderRight: '1px solid #e0e0e0' }}>
                            <input
                              ...
                              data-col={colIndex}
                              onFocus={e => {
                                setFocusedCell({ row: rowIdx, col: colIndex });
                                setTimeout(() => {
                              ...
                              onKeyDown={e => {
                                ...
                                handleCellKeyDown(e, rowIdx, colIndex);
                              }}
                              onWheel={e => {
                                if (wheelBlockFields.includes(field)) {
                                  (e.target as HTMLInputElement).blur();
                                }
                              }}
                              onChange={e => handleCellChange(rowIdx, field, e.target.value)}
                              autoFocus={!!(focusedCell && focusedCell.row === rowIdx && focusedCell.col === colIndex)}
                            />
                          </td>
                        );
                      })}
                      <td style={{ textAlign: 'center', borderRight: '1px solid #e0e0e0', background: 'rgba(46,125,50,0.2)', fontWeight: 'bold' }}>
                        {selectedDepartment === 'Bakery' ? getSoldQty(row.item) : row.sold}
                      </td>
                    </tr>
                  ))}
                </tbody>
