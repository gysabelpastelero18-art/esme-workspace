...
export default function InventoryPrintSummary() {
  ...
  const res = await fetch(`/api/warehouseInv?type=NF&${params.toString()}`);
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <button
          onClick={() => {
            ...
            window.location.href = `/warehouse-nonfood?group=NonFood&branch=Mayon&department=Warehouse&dateEncoded=${dateEncoded}`;
          }}
        ...
        <div className="header-buttons" style={{ display: 'flex', flexWrap: 'wrap', gap: '0.7rem', marginTop: '1rem', width: '100%', justifyContent: 'center' }}>
          ...
          <button
            ...
            style={{ background: 'linear-gradient(90deg,#3182ce,#38b2ac)', color: 'white', padding: '0.6rem 1.2rem', borderRadius: '8px', fontWeight: 'bold', fontSize: '1rem', boxShadow: '0 2px 8px rgba(49,130,206,0.15)', minWidth: '120px' }}
          >
            Print
          </button>
          <button
            onClick={handleDownloadImage}
            style={{ background: '#38b2ac', color: 'white', padding: '0.6rem 1.2rem', borderRadius: '8px', fontWeight: 'bold', fontSize: '1rem', boxShadow: '0 2px 8px rgba(49,130,206,0.15)', minWidth: '120px' }}
          >
            Download as Image
          </button>
          <button
            onClick={handleExportExcel}
            style={{ background: '#4299e1', color: 'white', padding: '0.6rem 1.2rem', borderRadius: '8px', fontWeight: 'bold', fontSize: '1rem', boxShadow: '0 2px 8px rgba(49,130,206,0.15)', minWidth: '120px' }}
          >
            Export to Excel
          </button>
        </div>
  ...
  {/* ...existing code... (remove duplicate header buttons above logo) ... */}
      <div className="header-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', borderBottom: '2px solid #38b2ac', paddingBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
            ...
            <div style={{ fontSize: '1.3rem', fontWeight: 600, color: '#38b2ac', marginLeft: '1.5rem' }}>NonFood</div>
          </div>
      ...
      {loading ? (
        ...
        categories.map(cat => (
          <div key={cat}>
            ...
            <SendExcelEmailButton tableData={Object.values(categoryMap[cat])} />
              <table className="category-table" style={{ width: '100%', minWidth: 400, borderCollapse: 'collapse', fontSize: '1rem', marginBottom: '2rem', tableLayout: 'fixed', boxShadow: '0 2px 8px rgba(49,130,206,0.08)', background: '#e6f4ea', borderRadius: '8px', overflow: 'hidden' }}>
                ...
                <tfoot>
                  <tr style={{ background: '#e6f4ea', fontWeight: 700 }}>
                    <td colSpan={3}></td>
                    <td style={{ border: '1px solid #a0aec0', borderTop: '3px solid #CBD5E1', padding: '10px', textAlign: 'right', background: 'inherit' }}>Total Amount:</td>
                    <td style={{ border: '1px solid #a0aec0', borderTop: '3px solid #CBD5E1', padding: '10px', textAlign: 'right', fontWeight: 700, background: 'inherit' }}>{categorySummary[cat].toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
